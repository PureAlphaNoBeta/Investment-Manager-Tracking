...
def run_etl_pipeline(update_13f=False):
    ...
    # 3. RUN DATA CLEANERS
    # ==========================================
    print(f"\n[{datetime.now()}] Running helper/data cleaner functions...")

    # Import and run the standardizer (unless names are already standardized)
    try:
        from data_cleaners.standardize_names import standardize_issuer_names
        standardize_issuer_names()
    except Exception as e:
        print(f"Error running standardize_names: {e}")

    # Import and run update_fund_addresses
    try:
        from data_cleaners.update_fund_addresses import update_funds_table
        update_funds_table()
    except Exception as e:
        print(f"Error running update_fund_addresses: {e}")

    # Import and run fix_database_sectors
    try:
        from data_cleaners.fix_database_sectors import main as fix_database_sectors_main
        fix_database_sectors_main()
    except Exception as e:
        print(f"Error running fix_database_sectors: {e}")
...
if __name__ == "__main__":
    ...
    run_etl_pipeline(update_13f=True)
